...
export default function PengurusDashboard() {
  ...
  // Pautan Kumpulan + Dokumen dari tetapan/home
  const [linkWasap,         setLinkWasap]         = useState('')
  const [tajukWasap,        setTajukWasap]        = useState('')
  const [arahanWasap,       setArahanWasap]       = useState('')
  const [linkTelegram,      setLinkTelegram]       = useState('')
  const [tajukTelegram,     setTajukTelegram]     = useState('')
  const [arahanTelegram,    setArahanTelegram]    = useState('')
  const [dokumenMuatTurun,  setDokumenMuatTurun]  = useState([])
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      // Fetch logos + pautan + dokumen dari tetapan/home
      try {
        ...
        if (homeSnap.exists()) {
          ...
          setLinkWasap(hd.linkWasap || '')
          setTajukWasap(hd.tajukWasap || '')
          setArahanWasap(hd.arahanWasap || '')
          setLinkTelegram(hd.linkTelegram || '')
          setTajukTelegram(hd.tajukTelegram || '')
          setArahanTelegram(hd.arahanTelegram || '')
          setDokumenMuatTurun((hd.dokumenMuatTurun || []).filter(x => x.nama && x.url))
        }
      } catch { /* abaikan */ }
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4 w-full">
      ...
      {/* Dokumen Muat Turun + Pautan Kumpulan */}
      {(dokumenMuatTurun.length > 0 || (linkWasap && tajukWasap) || (linkTelegram && tajukTelegram)) && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {linkWasap && tajukWasap && (
            <a href={linkWasap} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-3 p-4 bg-white border border-green-200 hover:border-green-400 hover:bg-green-50 rounded-xl transition-all shadow-sm group">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-gray-800 leading-tight">{tajukWasap}</p>
                {arahanWasap && <p className="text-xs text-gray-500 mt-0.5 leading-snug">{arahanWasap}</p>}
              </div>
              <svg className="w-4 h-4 text-gray-300 group-hover:text-green-500 shrink-0 transition-colors" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
              </svg>
            </a>
          )}
          {linkTelegram && tajukTelegram && (
            <a href={linkTelegram} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-3 p-4 bg-white border border-sky-200 hover:border-sky-400 hover:bg-sky-50 rounded-xl transition-all shadow-sm group">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-gray-800 leading-tight">{tajukTelegram}</p>
                {arahanTelegram && <p className="text-xs text-gray-500 mt-0.5 leading-snug">{arahanTelegram}</p>}
              </div>
              <svg className="w-4 h-4 text-gray-300 group-hover:text-sky-500 shrink-0 transition-colors" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
              </svg>
            </a>
          )}
          {dokumenMuatTurun.map((d, i) => (
            <a key={i} href={d.url} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-3 p-4 bg-white border border-gray-200 hover:border-[#003399] hover:bg-blue-50 rounded-xl transition-all shadow-sm group">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide">Muat Turun</p>
                <p className="text-sm font-bold text-gray-800 leading-tight">{d.nama}</p>
              </div>
              <svg className="w-4 h-4 text-gray-300 group-hover:text-[#003399] shrink-0 transition-colors" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
              </svg>
            </a>
          ))}
