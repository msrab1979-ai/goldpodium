/**
 * SijilPenyertaanPP — /:slug/pengurus/sijil-penyertaan
 * Pengurus Pasukan: muat turun sijil penyertaan atlet sekolah masing-masing.
 *
 * Sumber data:
 *   tenants/{schoolId}/tetapan/sijil          — template + kedudukan + gaya
 *   tenants/{schoolId}/kejohanan (aktif)      — namaKejohanan
 *   tenants/{schoolId}/atlet (kodSekolah==PP) — senarai atlet
 *   tenants/{schoolId}/kejohanan/{id}/pendaftaran — noBib
 */
...
export default function SijilPenyertaanPP() {
  ...
  async function load() {
    ...
    try {
      ...
      const pendSnap = await getDocs(
        query(collection(db, 'tenants', schoolId, 'kejohanan', kej.id, 'pendaftaran'), where('kodSekolah', '==', kodSekolah))
      )
      const bibMap = {}
  ...
  async function cetakSemua() {
    ...
    try {
      ...
      saveAs(content, `SIJIL_PENYERTAAN_${namaSek}.zip`)
    } catch (e) {
    ...
    return (
      <div className="p-6 flex items-center justify-center min-h-[200px]">
        <div className="text-center">
          ...
          <p className="text-xs text-gray-500 mt-2">Memuatkan data sijil...</p>
        </div>
      </div>
    )
  }
                  ...
                  <div key={atlet.noKP} className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        {atlet.noBib && (
                          <span className="text-[10px] bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded font-mono">{atlet.noBib}</span>
                        )}
                        {atlet.kategoriKod && (
                    ...
                    >
                      {downloading[atlet.noKP] ? <Spinner size="w-3.5 h-3.5" /> : '📜'}
